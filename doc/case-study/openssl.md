...
In this demo, we will explain how to use ESSTRA with a popular open-source package "OpenSSL". This document is structured as a step-by-step guide, allowing you to easily understand the basic operations of ESSTRA by following each step in order.

First, we shall build the OpenSSL package using the ESSTRA Core to generate the ELF files. These ELF files include metadata containing information about all the source files involved in the compilation, such as absolute paths and checksums. We will verify this using the ESSTRA Utility.
...
Finally, we calculate the effectiveness of ESSTRA in license analysis activity.

## ESSTRA Build Instructions

### Build and Install ESSTRA

Before you build the ESSTRA Core (esstracore.so) GCC plugin, you need to install a package on your system.
...
The version, at the time of writing, is 3.4.1, which is being used for the demo.
...
### Build the OpenSSL Package

We will follow the [official build instructions](https://github.com/openssl/openssl/blob/ddd7ecb04bcea5c13be3c73f3dc1a101087cdf24/INSTALL.md).

#### Install Prerequisites

For Debian/Ubuntu, the following command installs the required prerequisites mentioned in the [documentation](https://github.com/openssl/openssl/blob/ddd7ecb04bcea5c13be3c73f3dc1a101087cdf24/INSTALL.md).
...
We are going to make use of `BuildID` to find the ELF files.
A `BuildID` is a unique identifier assigned to a specific build of a binary and embedded within the ELF file itself.
...
For the demonstration, we take a single ELF file `openssl-3.4.1/apps/openssl` for simplicity.

The information embedded by ESSTRA would increase the size of the ELF files. We can reduce
the size of ELF files by eliminating duplication in the embedded information as below:

```sh
esstra$ du -sh ../openssl-3.4.1/apps/openssl
2.6M    ../openssl-3.4.1/apps/openssl

esstra$ python3 util/esstra.py shrink ../openssl-3.4.1/apps/openssl
...
```
Get the information embedded by ESSTRA as below:

```sh
esstra$ python3 util/esstra.py show ../openssl-3.4.1/apps/openssl
```
...
of each file. Below are the steps to add the license information of OpenSSL
source package to the metadata of the binaries/ELFs (generated during build)
using this feature.
...
I will be using version 4.1.0 for this demo.
...
Refer [here](https://github.com/sony/esstra/blob/main/samples/hello/README_FOSSOLOGY.md) to know step by step manual approach to analyze the source code with FOSSology and generate a report.

Here, we would use another approach to analyze the source code with FOSSology and generate a report using a tool [FOSSology.REST.shell](https://github.com/fossology/FOSSology.REST.shell)

#### Download FOSSology.REST.shell and Install Prerequisites

```sh
$ git clone https://github.com/fossology/FOSSology.REST.shell.git
$ apt install -y jq curl
```
#### Upload and Scan the OpenSSL Source
```sh
...
(snip)
  :
```
We would use the Upload id provided in the log to download the report.
...
```sh
esstra$ python3 util/esstra.py update ../openssl-3.4.1/apps/openssl -i SPDX2TV_openssl-3.4.1.tar.gz.spdx
* processing '../openssl-3.4.1/apps/openssl'...
* done.
...
of the ELF `openssl-3.4.1/apps/openssl`, type:
...
other than OpenSSL source files in the metadata of ELF `./openssl-3.4.1/apps/openssl`.
...
One way to measure effectiveness is to consider the reduction in the number of files for which manual effort is needed to review them. This reduction not only saves time but also enhance the quality by reducing human error.
...
Run ESSTRA util on all the generated ELF files, redirect output to a file.

```sh
openssl-3.4.1$ python3 ../esstra/util/esstra.py show $(find ./ -type f -exec sh -c "file '{}' | grep -q 'BuildID'" \; -print) >../esstra_scan_result.yaml
```
The above generated output has the details of all the source files used during the build including the system files. To find the unique source files of OpenSSL used during the build, we would exclude source files other than the OpenSSL source files.

Below we split all ELFs details in `esstra_scan_result.yaml` into separate yaml for each ELF and counts 'SHA1' under OpenSSL source directory to get unique source files used in all the ELFs.
...
1606
```
So, `1606` unique source files of OpenSSL were used during build to generate the ELF files.

### Calculating Effectiveness of ESSTRA

**Percentage of source files reported by ESSTRA can be calculated as below:**
...
This means out of all the files, only `16.685` % files were used in the generated ELFs.

## Summary

In this demo, we first compiled the OpenSSL source package using the ESSTRA Core
to generate the ELFs, and confirmed that the metadata in generated ELF
includes information about all the files involved in the compilation.

Next, we used the ESSTRA Utility to add license information to the metadata in the generated ELF.
...
Finally, we analyzed the effectiveness of ESSTRA in license analysis of the ELFs.
